...
const ContactDetailItem = ({ contacts, onClick }) => {
  ...
  // // }, []);
  console.log('contactParams=', params.contactId);

  console.log('contact', contacts);
  const id = '2e1cbf7d-7ef6-4926-a2fe-dca9de831235';
  return (
    <div>
      <ul className={styles.list}>
        {contacts.map(({ name, id, surname, tel }) => (
          <li key={id} className={styles.item}>
            <div className={styles.span_container}>
              <span className={styles.item_name}>{name}:</span>
              <span className={styles.item_name}>{surname}</span>{' '}
              <span className={styles.item_name}>{tel}</span>
            </div>
            <button
              id={id}
              type="button"
